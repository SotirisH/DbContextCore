...
namespace Mehdime.Entity
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public DbContextScope(bool readOnly, IDbContextFactory dbContextFactory)
        ...
        public DbContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, IDbContextFactory dbContextFactory)
        {
            ...
            {
                _nested = false;
                _dbContexts = new DbContextCollection(dbContextFactory, readOnly, isolationLevel );
            }
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            foreach (DbContext contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    var pknames=correspondingParentContext.Model.FindEntityType(toRefresh.GetType()).FindPrimaryKey().Properties.Select(x => x.Name);
                    var keyValues = new List<object>();
                    foreach (var pkname in pknames)
                    {
                        keyValues.Add(toRefresh.GetType().GetProperty(pkname).GetValue(toRefresh, null));
                    }

                    var entityOnParent = correspondingParentContext.Find(toRefresh.GetType(), keyValues.ToArray());

                    // Now we can see if that entity exists in the parent DbContext instance and refresh it.
                    if (entityOnParent!=null)
                    {
                        var entry = correspondingParentContext.Entry(entityOnParent);
...
" + Environment.StackTrace;
        ...
        private static AsyncLocal<InstanceIdentifier> AsyncLocal = new AsyncLocal<InstanceIdentifier>();
        ...
        internal static void SetAmbientScope(DbContextScope newAmbientScope)
        {
            ...
            // Replace Call context with AsyncLocal
            // https://stackoverflow.com/questions/41825375/why-asynclocal-is-different-from-callcontext
